...
class Task(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Setze die Position nur, wenn sie nicht explizit angegeben wurde
...
class Contact(models.Model):
    ...
    color = models.CharField(max_length=7, default='#ffa801')
    
    def save(self, *args, **kwargs):
        if not self.color:  # Nur wenn keine Farbe gesetzt ist
            self.color = self.generate_random_color()
        super().save(*args, **kwargs)

    def generate_random_color(self):
        return "#{:06x}".format(random.randint(0, 0xFFFFFF))  # Hex-Farbcode
